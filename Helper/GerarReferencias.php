...
class GerarReferencias
{
    private function formatNumber($number)
    {
        ...
        $nValorTmp = (strlen($valorTmp) - 1);

        for ($i = 1; $i != $nValorTmp; $i++) {
        ...
        if (strlen(strstr($valorTmp, '@')) > 0) {
            $valorTmp = trim(substr($valorTmp, 0, strpos($valorTmp, '@'))) .
                trim($sepDecimal) .
    ...
    public function generateMbRef($ent_id, $subent_id, $order_id, $order_value, $comEspacos)
    {
        ...
        if ($order_value >= 1000000) {
            return "<b>AVISO:</b> Pagamento fraccionado por exceder " .
                "o valor limite para pagamentos no sistema Multibanco<br>";
        }
